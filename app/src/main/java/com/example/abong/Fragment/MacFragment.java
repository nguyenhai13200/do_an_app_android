...
import com.example.abong.adapter.MacAdapter;
 ...
 * Use the {@link MacFragment#newInstance} factory method to
...
public class MacFragment extends Fragment {
    ...
    public static MacFragment newInstance(String param1, String param2) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mView = inflater.inflate(R.layout.fragment_mac, container, false);
        AnhXa();
        GetMac();
        AddToCart();
        loadMoreData();
    ...
    private void GetMac() {
        ApiService.apiService.getMac(page).enqueue(new Callback<ArrayList<Mac>>() {
            @Override
            public void onResponse(Call<ArrayList<Mac>> call, retrofit2.Response<ArrayList<Mac>> response) {
                if (response != null && response.body().size() != 0){
                    for (int i = 0; i < response.body().size();i++){
                        arrayMac.add(response.body().get(i));
                    }
            ...
            @Override
            public void onFailure(Call<ArrayList<Mac>> call, Throwable t) {
    ...
    private void AddToCart() {
        //sự kiện add cart
        macAdapter.setOnClickAddMacToCart(new ClickAddMacToCart() {
            @Override
            public void onClickAddMacToCart(ImageView imgAddCart, Mac mac) {
                    ...
                    public void onAnimationEnd(Animation animation) {
                        if (MainActivity.manggiohang.size() > 0){
                            ...
                            for (int i = 0; i < MainActivity.manggiohang.size(); i++){
                                if (MainActivity.manggiohang.get(i).getTensanpham().equals(mac.ten)){
                                    ...
                                    MainActivity.manggiohang.get(i).setGiasanpham(mac.getGia() * MainActivity.manggiohang.get(i).getSoluong());
                                    exists = true;
                                }
                            }
                            if (exists == false){
                                int soluong = 1;
                                MainActivity.manggiohang.add(new CartModle(mac.getId(),mac.getTen(), mac.getGia(), mac.getHinhanh(), soluong));
    ...
    private void AnhXa() {
        ...
        recyclerView = mView.findViewById(R.id.RecyclerviewMac);
        arrayMac = new ArrayList<>();
        progressBar = mView.findViewById(R.id.ProgressBar);
        recyclerView.setHasFixedSize(true);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL,false);
        recyclerView.setLayoutManager(linearLayoutManager);
        macAdapter = new MacAdapter(arrayMac,getContext());
